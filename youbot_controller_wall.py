...
def check_walls(wall):
    if "front" in wall and "left" in wall:
        return [-3, -3, -6.5, -6.5]
        # set_wheels(fr, br, fl, bl, -3, -3, -6.5, -6.5)
    if "front" in wall and "right" in wall:
        return [-6.5, -6.5, -3, -3]
        # set_wheels(fr, br, fl, bl, -6.5, -6.5, -3, -3)
    elif "back" in wall and "left" in wall:
        return [6.5, 6.5, 3, 3]
        # set_wheels(fr, br, fl, bl, 6.5, 6.5, 3, 3)
    elif "back" in wall and "right" in wall:
        return [3, 3, 6.5, 6.5]
        # set_wheels(fr, br, fl, bl, 3, 3, 6.5, 6.5)
    elif "front" in wall:
        return [-6.5, -6.5, -6.5, -6.5]
        # set_wheels(fr, br, fl, bl, -6.5, -6.5, -6.5, -6.5)
    elif "back" in wall:
        return [6.5, 6.5, 6.5, 6.5]
        # set_wheels(fr, br, fl, bl, 6.5, 6.5, 6.5, 6.5)
...
def wall_lookout(cameras, x_size, y_size, threshold):
    camera_dict = {"front": 0, "right": 0, "back": 0, "left": 0}
    walled = []
    for image_array, camera in enumerate(camera_dict):
        filtered_array, filtered_pos = filter_image_array(cameras[image_array], x_size, y_size)

        for idx in range(len(filtered_array)):
            if is_pixel_match(filtered_array[idx],visual_dict["Wall_bright"]) or is_pixel_match(filtered_array[idx], visual_dict["Wall_shadow"]):
                camera_dict[camera] += 1.0

    for camera in camera_dict:
        print(camera_dict)
        if camera_dict[camera] > threshold:
            walled.append(camera)
    return walled
    ...
    while(robot_not_dead == 1):
     ...
     # The following code is called every timestep:
        ...
        if timer%2 == 0 and emergency == 0:
            ...
            wall = wall_lookout([front_RGB, right_RGB, back_RGB, left_RGB], 128, 64, 6000)
            speeds = check_walls(wall)
            if speeds:
                set_wheels(fr, br, fl, bl, speeds[0], speeds[1], speeds[2], speeds[3])
        ...
        if emergency > 0:
            ...
            if (prev_health - robot_info[0]) >= 1:
                # at this point, prev_health is health before emergency declared
                # if there is a difference, means zombie still there --> likely at wall
                set_wheels(fr, br, fl, bl, -6.5, -6.5, 6.5, 6.5)
                prev_health = robot_info[0]
            else:
                # regular escape, backwards
                set_wheels(fr, br, fl, bl, -6.5, -6.5, -6.5, -6.5)
